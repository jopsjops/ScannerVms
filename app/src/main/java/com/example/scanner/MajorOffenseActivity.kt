...
class MajorOffenseActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_major_form)

        findViewById<TextView>(R.id.title).text = "Offense: Major"
        ...
        // Offense options
        val offenseSpinner: Spinner = findViewById(R.id.majorOff_spinner)
        val offenses = arrayOf(
            "Cheating", "Forgery & Plagiarism", "False Representation", "Defamation",
            "Substance Influence", "Unauthorized Entry", "Theft", "Drug Possession/Use",
            "Insubordination", "Physical Injury", "Threats & Bullying", "Gambling",
            "Hazing", "Unauthorized Name Use", "Financial Misconduct", "Unauthorized Sales",
            "Extortion", "Vandalism", "Degrading Treatment", "Deadly Weapons",
        ...
        submitButton.setOnClickListener {
            ...
            if (selectedOffense.isBlank()) {
                Toast.makeText(this, "Please select a major offense", Toast.LENGTH_SHORT).show()
            } else {
                val intent = Intent(this, DetailsFormActivity::class.java).apply {
                    ...
                    putExtra("major", selectedOffense)
                    putExtra("date", date)
                    putExtra("time", time)
